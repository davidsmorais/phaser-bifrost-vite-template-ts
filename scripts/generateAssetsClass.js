...
function generateAssetOptionsSpriteSheet(assetName, extensions) {
  ...
  if (prop.length < 2 || prop.length > 5) {
    console.error(
      "Invalid number of Spritesheet properties provided for '" +
        i +
...
function generateAssetOptionsAtlas(assetName, extensions) {
  ...
  const parseFrame = (frameFull, name) => {
    const frame = frameFull?.split?.(".")?.[0] ?? name;
    return {
      ...
      if (asset.files) {
        for (const file of asset.files) {
          s.push(
            `import ${
              asset.className
            }${file.extName.toUpperCase()} from "assets/${file.assetName}.${
              file.extName
            }";`
          );
        }
